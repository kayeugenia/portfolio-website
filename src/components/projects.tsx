	...
	return (
		<section className="border-[1px] border-gray-200 rounded-xl p-5 my-3 xl:my-5 xl:mr-3 sm:max-w-lg md:max-w-full xl:max-w-sm flex flex-col md:flex-row xl:flex-col hover:transform-gpu hover:scale-105 transition-all">
			<Image
				...
				height={0}
				sizes="100vw"
				className="flex-2 shadow-md rounded-lg w-auto h-auto md:w-[50%] xl:w-auto md:mt-auto md:mb-auto xl:mt-0 xl:mb-0"
			/>
			<div className="flex-2 md:ml-6 xl:ml-0">
				<h3 className="text-xl font-[500] mt-2">{title}</h3>
				<p className="text-sm mt-2">{description}</p>
				<ul className="flex flex-wrap mt-4 gap-2">
					{tags.map((tag, index) => (
						<li className="text-xs font-mono mx-1" key={index}>
							{tag}
						</li>
					))}
				</ul>
					...
					{githubUrl && (
						<a href={githubUrl} target="_blank" rel="noopener noreferrer">
							<img
								...
								alt="GitHub Link"
								className="h-6 w-6 mr-4 hover:transform-gpu hover:scale-110"
							/>
						</a>
					)}
					{webUrl && (
						<a href={webUrl} target="_blank" rel="noopener noreferrer">
							<img
								src="/images/link.png"
								alt="Web Link"
								className="h-5 w-5 mt-[0.15rem] hover:transform-gpu hover:scale-110"
							/>
						</a>
					)}
				</div>
