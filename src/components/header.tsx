...
export default function Header() {
    ...
    return (
        <header className="z-[999] relative">
            ...
            <motion.nav className="flex fixed top-[0.15rem] left-1/2 h-12 py-2 w-[60%] sm:top-[1.7rem] sm:h-[initial] sm:py-0 sm:w-auto"
                initial={{y: -100, x:"-50%", opacity:0 }}
                animate={{y: 0, x:"-50%", opacity: 1}}>
                <ul className="flex w-[20rem] flex-wrap items-center justify-center gap-y-1 text-[0.9rem] font-medium text-zinc-400 sm:w-[initial] sm:flex-nowrap sm:gap-5">
                    {
                        links.map(link => (
